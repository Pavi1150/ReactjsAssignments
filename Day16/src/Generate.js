...
function Generate() {
  ...
  const handleClick = (e) => {
    ...
    setText(datas.slice(0, para))
  }
  return (
    <div>
      <div className='h3 fw-bold text-center pt-5 h'>TIRED OF BORING LOREM IPSUM?</div>
      <div className='text-center'>
        <label className="fs-4 pt-5 f">Paragraph:</label>
        <input type="number" id="para" value={count} onChange={(e) => setCount(e.target.value)} />
        <button className='btn btn-primary' onClick={handleClick}>Generate</button>
        {text.map(data => <div key={data.id} className="container col-5 pt-5 text-secondary">
          {data}
        </div>)}
      </div>
    </div>
